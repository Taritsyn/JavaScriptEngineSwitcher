...
namespace JavaScriptEngineSwitcher.ChakraCore
{
	...
	internal sealed class ScriptDispatcher : IDisposable
	{
		...
		public T Invoke<T>(Func<T> func)
		{
			...
			ScriptTaskWithResult<T> task;

			using (var waitHandle = new ManualResetEvent(false))
			{
				task = new ScriptTaskWithResult<T>(func, waitHandle);
				EnqueueTask(task);
...
#error No implementation for this target
		...
		public void Invoke(Action action)
		{
			...
			ScriptTaskWithoutResult task;

			using (var waitHandle = new ManualResetEvent(false))
			{
				task = new ScriptTaskWithoutResult(action, waitHandle);
				EnqueueTask(task);
...
#error No implementation for this target
		...
		private sealed class ScriptTaskWithResult<T> : ScriptTask
		{
			...
			/// <param name="func">Delegate to invocation</param>
			/// <param name="waitHandle">Event to signal when the invocation of delegate has completed</param>
			public ScriptTaskWithResult(Func<T> func, ManualResetEvent waitHandle)
				: base(waitHandle)
			{
				_func = func;
			}
		...
		private sealed class ScriptTaskWithoutResult : ScriptTask
		{
			...
			/// <param name="action">Delegate to invocation</param>
			/// <param name="waitHandle">Event to signal when the invocation of delegate has completed</param>
			public ScriptTaskWithoutResult(Action action, ManualResetEvent waitHandle)
				: base(waitHandle)
			{
				_action = action;
			}

...
namespace JavaScriptEngineSwitcher.NiL
{
	...
	public sealed class NiLJsEngine : JsEngineBase
	{
		...
		/// </summary>
		private const string EngineVersion = "2.5.1339";
		...
		{
			OriginalScript parsedScript;

			try
			{
				parsedScript = OriginalScript.Parse(code);
			}
			...
			return new NiLPrecompiledScript(parsedScript);
		}
		...
		protected override void InnerExecute(IPrecompiledScript precompiledScript)
		{
			...
			try
			{
				lock (_synchronizer)
				{
					nilPrecompiledScript.ParsedScript.Evaluate(_jsContext);
				}
			}
			catch (OriginalException e)
			{
